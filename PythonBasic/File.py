...
StationFares = []
with open("C:\DL\FastCampus\PythonBasic\Stationfares.json", 'r') as station_fares_file:
    StationFares = json.load(station_fares_file)
...
def save_content():
    with open("C:\DL\FastCampus\PythonBasic\Stationfares.json", 'w', encoding="UTF-8") as station_fares_file:
        jsonString = json.dumps(StationFares, ensure_ascii=False)
        station_fares_file.write(jsonString)
    station_fares_file.close()
